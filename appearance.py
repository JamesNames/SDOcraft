...
class PhotoPanel(tk.Canvas):
    ...
    def render(self):
        ...
        if self.original is not None:
            ...
            self.create_text(width//2, height//2+50, text='Фото загрузится из интернета', fill=MUTED, font=(FONT, 11))


def build(app):
    root = app.root
    root.configure(bg=BG)
    root.title('SDOcraft • Твоя команда. Твой мир.')
    ...
    label(rail, 'ТЕРРИТОРИЯ СВОИХ', 9, MUTED).pack(anchor='w', pady=(3, 21))
    ...
    label(nav, '  /  ГЛАВНАЯ', 10, ACCENT, True).pack(anchor='w', padx=9, pady=10)
    label(rail, 'ПРОФИЛЬ ИГРОКА', 9, MUTED, True).pack(anchor='w', pady=(24, 10))
    for key, title, variable in [
        ('nickname', 'Ник', app.vars['nickname']),
    ...
    label(rail, 'ТВОЙ ОТРЯД ЖДЁТ', 9, ACCENT, True).pack(anchor='w')
    desc = label(rail, 'Собери команду.\nЗайми свою позицию.\nСоздай свою историю.', 10, MUTED)
    ...
    label(footer, 'Вход по нику · без Microsoft', 8, MUTED).pack(anchor='w', pady=(5, 0))

    main = tk.Frame(shell, bg=BG)
    main.pack(side='left', fill='both', expand=True, padx=25, pady=24)
    header = tk.Frame(main, bg=BG)
    header.pack(fill='x')
    label(header, 'ГОТОВ К ИГРЕ?', 10, ACCENT, True).pack(side='left')
    label(header, 'SDO  /  СЕТЕВАЯ ИГРА', 9, MUTED).pack(side='right')
    ...
    for title, command in [
        ('Папка игры', lambda: app.open_folder(app.game_folder)),
        ('Добавить моды', app.add_mods),
        ('Проверить сборку', app.check_mods),
        ('Восстановить', lambda: app.start(True)),
    ]:
    ...
    label(note, 'ВСЁ ДЛЯ СТАРТА', 9, ACCENT, True).pack(anchor='w')
    label(note, 'Игра, Java и моды установятся автоматически.', 9, MUTED).pack(anchor='w', pady=(4, 0))
    app.play = button(launch, 'ИГРАТЬ  →', lambda: app.start(False), primary=True)
    ...
    app.status = tk.StringVar(value='Готово к запуску. Для первой установки нужен интернет.')
    ...
    def toggle():
        if log_frame.winfo_manager():
            ...
            logs.configure(text='+ Журнал запуска')
        else:
            log_frame.pack(fill='x', pady=(5, 0))
            logs.configure(text='− Скрыть журнал')
    logs = tk.Button(main, text='+ Журнал запуска', command=toggle, bg=BG, fg=MUTED,
